🌟 weiweiwei 內部系統操作指南 (v11.18.20)
歡迎使用 weiweiwei 的專屬 ERP 系統！這套系統的核心目標是讓每一件衣服從「排程製作」到「在庫調度」，最後到「順利售出」的過程都清清楚楚。

一、 系統權限與基本認知
切換身份：進入系統後，請先在右上角確認自己的身份。

Admin (Rie & Raina)：擁有最高權限，可看總覽數據、調整系統設定與新增工單。

Maker (例如 Kim, Kelly)：專注於「生產排程」，只能看到與自己相關的製作任務，版面更簡潔。

二、 生產排程 (製作團隊日常操作)
當有新的工單建立後，商品會出現在「生產排程」板塊。

查看任務：利用上方的下拉選單過濾「自己的名字」，找出分配給你的製作任務。

更新進度：點擊列表最右側的「齒輪按鈕 ⚙️」，可以更改該批商品的狀態。

流程規範：Pending (待定) ➔ To Make (準備製作) ➔ Making (製作中) ➔ QC (品檢) ➔ Ready (完工入庫)。

完工入庫：當狀態改為 Ready 並設定好「入庫地 (JB Studio 或 PNG Studio)」後，該商品就會正式進入庫存看板，交接給理貨人員。

Style SKU 與 Garment ID：新增工單時先選擇既有 Style SKU，例如 Long Wrap Skirt Original 使用 `2BTM005`。系統會為該工單中的每一件實物建立永久 ID，例如 `2BTM005-001`、`2BTM005-002`。舊工單不會被自動批次修改；在編輯舊工單並確認 Style SKU 後，系統才會為該批所有單品（包括 Sold）補上 ID。已建立 Garment ID 的 Style SKU 不能再更改。

三、 庫存與調貨管理 (日常出貨/理貨必看)
這是系統最靈活的地方，請務必了解「實體地點」與「線上商店」的雙重邏輯。

看懂狀態標籤：

卡片上的顏色標籤代表它實體在哪裡（JB Studio、PNG Studio 或是外部 Stockist）。

如果標籤前面帶有 🌐 符號（例如 🌐 JB Studio），代表這件衣服「實體躺在 JB，但已經同步刊登上 Online 網店」。

一鍵出貨 / 調貨 (最常用)：
不需要點進商品，直接在單品卡片下方的下拉選單操作：

上線販售：選擇 🌐 上架至 Online。

跨店調貨：選擇 JB Studio 或 PNG Studio（系統會自動將商品調過去，若原本是 Online 狀態也會無縫保留）。

發給寄賣點：選擇外部的 Stockist 名稱。

四、 單品進階編輯 (特例處理)
如果遇到「同一批工單中，某一件衣服特別不一樣」的狀況，請點擊該單品的圖片，打開「詳細資訊」彈窗：

獨立定價：如果這件衣服因為微瑕疵要打折，或有特殊定價，可以在「獨立售價 (RM)」輸入新價格，它會直接覆蓋原本的預設主定價。

更換單品圖：如果這件衣服有單獨重拍的照片，可以在這裡上傳替換。

單品備註：可記錄「微瑕疵」、「VIP保留款」等重要備註。有備註的商品，列表卡片右上角會出現一個紅點提示大家注意。

單品 History：系統會自動記錄之後发生的地點轉移、售出、售出更正與取消售出。舊商品不需要批次轉換；尚未发生新操作時顯示「尚無記錄」是正常的。

五、 售出結帳 (Mark as Sold)
當商品確認售出並收到款項後：

點擊商品圖片進入詳細資訊。

點擊最下方的黑色按鈕 「標記為已售出」。

輸入最終成交價（系統會自動帶入原本的定價，若有折扣請自行修改金額），並選好售出日期。

按下確認後，該商品會變成灰色 SOLD 狀態，並正式計入當月的銷售業績中。

總覽數字：首頁的「目前可售库存」只计算已完成、未售出且已有地点的单品；「生产流程中」包含 Pending、To Make、Making 与 QC。「需要处理：无地点」不会把仍在生产的商品算进去。卡片下方另显示系统内全部单品记录数，避免把总记录误认为当前库存。

更正售出：重新打開 SOLD 商品後可直接修改成交價、付款方式、日期與 Sales Note，不必先取消售出。只有真的要讓商品返回庫存時才使用「取消售出」。

Popup 活動：可在「設定 → 當前 Popup 銷售活動」修改活動名稱、售出地點及日期。設定只影響之後的新銷售，不會改寫舊活動；舊 Popup 仍可在銷售報表的活動選單查看。

六、 防呆與救援須知 (🚨 重要)
照片上傳請耐心等候：新增工單或替換照片時，按下儲存後按鈕會顯示「處理中...」，請給系統 1~3 秒鐘將照片傳上雲端，千萬不要急著重整網頁。

找不到商品怎麼辦？：如果操作太快，不小心把實體地點跟 Online 都取消掉了，商品會跑到總覽看板的**「無地點標記 (遺漏件)」**區。只要點開該區域，重新賦予它地點即可救援成功。

旧资料工具：Style SKU 整理与旧商品编号补齐已收在设置页的「旧资料维护工具」。平时不会自动扫描；只有展开该区块时才会准备预览，减少日常打开设置页的等待。

完整资料备份：在「设置 → 完整资料备份」下载 JSON。档案包含所有现有与封存工单、Sold 资料、单品 History、系统设置、Style SKU、Garment ID 计数器及照片网址；它不会修改 Firebase。照片本体仍存放在 Firebase Storage，JSON 只保存照片网址，因此请同时保留 Firebase 专案，并把备份档放在安全位置。建议在 Popup 前及大量修改资料前各下载一次。

销售写入确认：按「确认售出」后，要等页面明确显示绿色成功讯息才算完成。离线或 Firebase 写入失败时，售出视窗会保留并显示红色错误；请恢复网络后在同一视窗重试，不要把失败讯息视为已记录。

快速售出与统一报表：库存卡片可直接点「售出」开启原有结帐视窗，仍会经过相同的版本锁与 Firebase 成功确认。首页 Sales Reports 可切换指定 Popup、所有 Popup 或所有销售，并按日期、付款方式与币别筛选；MYR 与 SGD 永远分开统计，不会换算或相加。

销售商品图片：Sales Report 每笔商品优先载入现有缩图，没有缩图才使用原图，并采用 lazy loading；点缩图可放大查看，没有照片则显示 No Image。CSV 会附上图片网址，但不会把图片嵌入档案。
